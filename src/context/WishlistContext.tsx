...
export const WishlistProvider = ({ children }: { children: React.ReactNode }) => {
  const { token } = useAuth(); // ✅ الـ token من AuthContext
  const [wishlist, setWishlist] = useState<WishlistItem[]>([]);
  const [wishlistIds, setWishlistIds] = useState<string[]>([]);
  const [loading, setLoading] = useState(true);
  const [initialized, setInitialized] = useState(false);
  ...
  const getWishlist = async () => {
    // ✅ لو مفيش token، مجبش الـ wishlist
    if (!token) {
      setWishlist([]);
      setWishlistIds([]);
      setLoading(false);
  ...
  // ✅ بيتنفذ كل ما الـ token يتغير (login / logout)
  useEffect(() => {
    getWishlist();
  }, [token]);

  const addToWishlist = async (productId: string) => {
    // ✅ Guard: لو مش مسجل دخول
    if (!token) {
  ...
  return (
    <WishlistContext.Provider
      value={{ wishlist, wishlistIds, addToWishlist, removeFromWishlist, isInWishlist, loading, initialized }}
    >
      {children}
    </WishlistContext.Provider>
  );
